...
└──────────────────────┬─────────────────────────────────────────┘
          ...
          ┌────────────▼────────────┐
          │  パス 2                 │
          │  背景レイヤーデータ取得  │  pcall { obj.copybuffer, obj.getpixeldata }
...
各パスは `@scene_lighting.anm2` 内の独立した論理セクションとして実装されており、
コメントでパス番号と役割を明記している。
...
```
1. obj.copybuffer("tmp", "obj:<abs_layer>")  ← AviUtl2 推奨パス
   obj.getpixeldata("tmp") で取得

2. obj.copybuffer("tmp", "frm:0")            ← シーンフレームバッファ
   obj.getpixeldata("tmp") で取得

3. フォールバック: amb = (128, 128, 128)     ← バッファ取得不可時
```
